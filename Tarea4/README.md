...
**Descripcion**: Realizar utilizando CUDA con C++ el script para generar una suma y resta de imagenes
...
4. Reducimos el tamaño de las imagenes para facilitar el proceso y que ambas sean de la misma dimensión

4. Dividimos las imagenes en tres matrices R G y B

5. Declaramos la función CUDA para poder sumar/restar las matrices usando: R(x, y):= (A(x,y)+B(x,y))/2 donde la matriz resultante es la división entre dos de la suma de las dos matrices. Este proceso se aplica 3 veces, una para cada suma de matriz, digamos la matriz de canal R se suma con la otra matriz de canal R, lo mismo para las otras 4 matrices de canal G y B restantes.

6. Declaramos las filas y columnas y tamaño de las matrices

7. Copiamos la matriz a una variable declarada en cuda/c++
